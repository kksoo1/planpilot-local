...
다음 코드 작업에서는 전체 오류/로딩 체계를 만들지 않고 기존 화면의 최소 empty 상태 문구와 조건만 점검/보강한다. JSON export는 그 다음 단계에서 export 전용 성공/실패 메시지 정책을 붙여 검토한다.

## Next Task Scope
- 우선 후보: 업무 없음, 검색 결과 없음, 필터 결과 없음, 추천 업무 없음, 지난 마감/7일 이내 마감 업무 없음 상태 점검
- 보조 후보: JSON export 전용 성공/실패 메시지 정책 정리
