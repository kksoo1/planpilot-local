...
JSON import/복원은 아직 구현하지 않았고, 정책상 1차 구현 후보는 실제 DB 반영이 아닌 파일 검증 또는 미리보기다.
...
다음 작업에서는 JSON import 파일 검증 유틸을 실제로 구현할지 검토한다. 구현하더라도 DB 반영은 하지 않고, 검증 함수 또는 미리보기 정책 범위로 제한하는 편이 안전하다.

## Next Task Scope
- 우선 후보: JSON import 파일 검증 유틸 구현 여부 검토
- 보조 후보: 검증 결과 미리보기 UI 정책 구체화
