...
- 가져오기/복원 정책 문서: `docs/data-import-restore-policy.md`
- 1차 내보내기 대상: `tasks`, `projects`, `appSettings`
- 1차 형식 후보: 사람이 읽기 쉬운 JSON
- 가져오기/복원은 데이터 손상 위험이 있어 내보내기 이후 별도 단계로 검토하며, 1차 후보는 실제 DB 반영이 아니라 파일 검증 또는 미리보기다.
...
최근 리팩터링 기준으로 업무 form state와 업무 action orchestration은 `src/hooks`로 일부 분리되어 있습니다.
