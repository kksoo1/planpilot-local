...
`useTaskFormState` 분리는 완료되었고, 업무 추가/수정 form 입력값, 수정 대상 id, 추가 form 열림 상태, reset/start/cancel 흐름은 hook이 담당한다. 현재 업무 추가/수정 submit, 완료/미완료 토글, 업무 삭제 orchestration은 `useTaskActions`가 담당한다. 이 섹션은 `useTaskActions`를 만들 때 고정했던 순서와 범위의 기록으로 유지한다.
...
- 완료/미완료 토글과 업무 삭제는 현재 `useTaskActions`에 포함되어 있다.
- 삭제 handler와 완료 토글은 수동 테스트 체크리스트 기준으로 회귀 여부를 확인한다.
...
`useTaskFormState`와 `useTaskActions`가 추가되어 업무 영역의 form state, 추가/수정 submit orchestration, 완료/미완료 토글, 업무 삭제는 `App.tsx` 밖으로 일부 분리되었다.
...
- `useTaskActions`
  ...
  - 업무 삭제 확인창을 표시하고, 확인한 경우에만 `deleteTask(task.id)`를 호출한다.
...
업무 삭제 handler를 `useTaskActions`로 이동한 뒤 확인할 이유:
...
- 삭제 handler 항목도 현재 `useTaskActions` 회귀 검증 항목으로 본다.
- 다음 코드 분리 전에는 업무 추가/수정/삭제/완료 토글 전체 항목을 실제 화면에서 먼저 확인한다.

## 34. 업무 삭제 handler 분리 후 기준

현재 `handleDeleteTask`는 `useTaskActions`로 이동했다. 이 handler는 단순히 `deleteTask`를 호출하는 함수가 아니라, 삭제 확인창과 실제 삭제 실행 시점을 함께 조율한다.
...
`useTaskActions` 이동 후 회귀 확인이 필요한 이유:
...
## 36. 업무 hook/action 현재 구조

현재 업무 영역은 form state와 action orchestration이 다음처럼 분리되어 있다.

- `useTaskFormState`
  - 업무 추가 form state를 담당한다.
  - 업무 수정 form state를 담당한다.
...
- `useTaskActions`
  ...
  - 완료/미완료 토글을 담당한다.
  - 업무 삭제를 담당한다.
  - 삭제 확인창을 표시하고, 사용자가 확인한 경우에만 `deleteTask(task.id)`를 호출한다.

`App.tsx`에 남아 있는 업무 관련 책임:

- `useTaskFormState`와 `useTaskActions`에 필요한 store action, form state, reset 함수를 조립한다.
- `TasksView`에 업무 form state, filter state, sort state, handler props를 전달한다.
- 앱 탭 상태, 검색/필터/정렬 상태, Today 화면용 파생 데이터와 추천 업무 상태를 조립한다.
- `handleStartEditTask`, `handleCancelEditTask`처럼 form state hook을 호출하는 얇은 wrapper를 유지한다.

`App.tsx`에서 제거된 업무 관련 책임:

- 업무 추가 submit 직접 처리
- 업무 수정 submit 직접 처리
- 완료/미완료 토글 직접 처리
- 업무 삭제 확인창과 `deleteTask(task.id)` 직접 호출
...
## 39. MVP 수동 회귀 테스트 결과 기록

MVP 완료 후보 상태를 실제 완료로 판정하려면 `docs/manual-test-checklist.md` 기준으로 사람이 직접 화면을 확인하고, 결과를 `docs/manual-regression-test-result.md`에 기록한다.

현재 상태:

- `docs/manual-regression-test-result.md` 결과 기록 템플릿을 준비했다.
- 전체 결과 기본값은 `미수행`으로 둔다.
- 실제 브라우저 실행, `npm run dev`, preview, 수동 화면 확인은 아직 수행하지 않았다.
- 따라서 MVP는 계속 "완료 후보 / 수동 회귀 테스트 결과 기록 필요" 상태로 둔다.
...
1. `npm run build` 성공 상태를 먼저 확인한다.
2. 앱을 실행할 수 있는 환경에서 `docs/manual-test-checklist.md`를 따라 화면별 수동 테스트를 수행한다.
3. 각 범위의 결과와 발견 이슈를 `docs/manual-regression-test-result.md`에 기록한다.
4. 하나라도 실패가 있으면 MVP 완료로 판정하지 않고, 실패 항목을 다음 수정 작업 후보로 올린다.
5. 모든 핵심 항목이 통과하면 ROADMAP과 memory-bank에 MVP 완료 판정 결과를 반영한다.

## 40. MVP 수동 회귀 테스트 결과 정리

`docs/manual-regression-test-result.md`를 확인한 결과, 현재 문서는 아직 실제 수동 테스트 결과가 기록되지 않은 템플릿 상태다.
...
- 아직 MVP 완료로 확정하지 않는다.
- 통과로 기록된 핵심 항목이 없으므로 "MVP 완료 후보 / 수동 회귀 테스트 미수행" 상태로 유지한다.
- 실패 항목도 아직 기록되지 않았으므로 수정 필요 항목은 새로 만들지 않는다.

다음 필요 작업:

1. 실제 앱 화면에서 `docs/manual-test-checklist.md`의 MVP 핵심 항목을 수동으로 확인한다.
2. 확인 결과를 `docs/manual-regression-test-result.md`에 통과/실패/보류/미수행으로 기록한다.
3. 실패 또는 보류 항목이 있으면 ROADMAP에 MVP 완료 전 수정 필요 항목으로 정리한다.
4. 모든 핵심 항목이 통과하면 MVP 완료 확정 후보로 상태를 갱신한다.

이전 재확인 기록:

- `docs/manual-regression-test-result.md`를 다시 확인했지만, 통과/실패/보류로 기록된 실제 테스트 결과는 아직 없다.
- 모든 화면별 결과와 최종 판정이 `미수행`이므로 MVP 완료 상태는 변경하지 않는다.
- 통과로 명시되지 않은 항목을 임의로 통과 처리하지 않는다.
- 최신 수동 테스트 결과 반영은 아래 "41. MVP 수동 회귀 테스트 결과 반영"을 기준으로 한다.

## 41. MVP 수동 회귀 테스트 결과 반영

`docs/manual-regression-test-result.md`에 기록된 최신 수동 테스트 결과를 기준으로 MVP 상태를 다시 정리했다.
...
- 프로젝트 화면
  ...
  - 기본 프로젝트 삭제 방지
  - 업무 연결 프로젝트 삭제 방지
  - 프로젝트별 전체/완료/미완료 업무 수
- 설정 화면
  - 현재 설정 상태 읽기 전용 표시
  - theme, language, AI Provider, 알림, 첫 실행 완료 상태 읽기 전용 표시
  - 설정 화면 진입만으로 `updatedAt`이 변경되지 않음
  - 알림 권한 요청 없음
- 데이터 유지
  - 새로고침 후 업무, 프로젝트, 설정 유지
  - 브라우저 재실행 후 IndexedDB 데이터 유지
...
- Today 화면의 추천 업무가 rule-based 기준에 맞게 표시되는지 확인
- 전체 업무 화면의 완료 업무 표시/숨김 동작 확인
...
- 현재 MVP 완료 판정은 `보류`다.
- 핵심 항목 중 보류가 남아 있으므로 MVP 완료 확정으로 처리하지 않는다.
- 보류 항목을 실제 화면에서 확인해 `통과` 또는 `실패`로 기록하기 전까지는 "MVP 완료 후보 / 보류 항목 확인 필요" 상태로 둔다.

MVP 완료 전 확인 필요 항목:

1. 추천 업무가 rule-based 기준에 맞게 표시되는지 확인한다.
2. 완료 업무 표시/숨김이 기대대로 동작하는지 확인한다.
3. 발견 이슈와 후속 조치 항목을 비워둘지, "없음" 또는 구체 항목으로 기록할지 정리한다.
4. 위 항목이 모두 통과 또는 이슈 없음으로 정리되면 MVP 완료 확정 후보로 다시 판정한다.

## 42. MVP 수동 회귀 테스트 보류 항목 재확인 결과

`docs/manual-regression-test-result.md` 기준으로 이전 보류 항목 2개가 다시 확인되었다.

새로 통과로 기록된 항목:

- Today 화면의 추천 업무가 rule-based 기준에 맞게 표시된다.
- 전체 업무 화면의 완료 업무 표시/숨김이 동작한다.

현재 통과로 보는 핵심 기능 범위:

- Today 화면의 전체 업무 수, 완료 업무 수, 지난 마감 업무, 7일 이내 마감 업무, 추천 업무, 완료 업무 제외 기준
...
- 핵심 기능 항목은 모두 통과로 기록되었다.
- 그러나 문서에 `미수행`으로 남은 항목이 있으므로 아직 MVP 완료 확정으로 처리하지 않는다.
- 위 미수행 항목이 "없음" 또는 실제 값으로 정리되면 MVP 완료 확정 후보로 갱신한다.

## 43. MVP 완료 확정

`docs/manual-regression-test-result.md`의 남은 메타 항목과 후속 기록이 정리되어, 현재 기준으로 PlanPilot Local MVP를 완료로 판정한다.
...
- 전체 결과: 통과
- 발견 이슈: 없음
- 후속 조치: MVP 이후 기능 고도화 진행

MVP 완료 근거:

- Today 화면 핵심 항목이 모두 통과로 기록되었다.
- 전체 업무 화면의 CRUD, 완료/미완료 토글, 검색, 완료 업무 표시/숨김, 프로젝트 필터, 마감일 정렬이 통과로 기록되었다.
- 프로젝트 화면의 CRUD, 기본 프로젝트 삭제 방지, 업무 연결 프로젝트 삭제 방지, 프로젝트별 통계가 통과로 기록되었다.
- 설정 화면의 읽기 전용 상태 표시와 알림 권한 요청 없음이 통과로 기록되었다.
- IndexedDB 데이터 유지가 통과로 기록되었다.
- 서버 API, `localStorage`, 로그인, 클라우드 동기화, Notification 권한 요청, Capacitor, 외부 AI API 미추가가 통과로 기록되었다.
- 실패, 보류, 미수행 항목이 남아 있지 않다.

MVP 이후 작업 후보:

1. 데이터 백업/내보내기 정책을 문서화한다.
2. 오류/로딩 상태 보강 범위를 문서화한다.
3. 업무 필터/정렬 UX 개선 후보를 정리한다.
4. IndexedDB schema migration 실전 절차를 문서화한다.
5. 기능 고도화 단계 진입 전에도 서버 API, 로그인, 클라우드 동기화, 알림, Capacitor, 외부 AI API 추가 여부는 별도 정책 검토 후 결정한다.

## 44. 데이터 백업/내보내기 정책

MVP 이후 첫 기능 고도화 후보는 데이터 백업/내보내기다. 실제 구현 전 정책은 [docs/data-backup-export-policy.md](docs/data-backup-export-policy.md)를 기준으로 한다.

현재 결정:

- 이번 단계에서는 기능을 구현하지 않고 정책만 문서화한다.
- 1차 대상 데이터는 `tasks`, `projects`, `appSettings`다.
- 1차 내보내기 형식 후보는 사람이 읽기 쉬운 JSON이다.
- CSV는 업무 목록 중심의 보조 내보내기 후보로만 둔다.
- 가져오기/복원은 데이터 손상 위험이 있으므로 내보내기 이후 별도 단계로 분리한다.
- 1차 구현 우선순위는 "JSON 내보내기만 먼저"다.
...
- 로그인, 클라우드 동기화, 알림 권한 요청, Capacitor를 추가하지 않는다.
- 외부 AI API로 백업 데이터를 보내지 않는다.

가져오기/복원 전 확인할 정책:

- 전체 덮어쓰기와 병합 중 어떤 방식을 먼저 지원할지 결정한다.
- 중복 ID 처리 방식을 결정한다.
- 가져오기 전 기존 데이터 자동 백업 여부를 결정한다.
- `appSettings` 포함 여부와 `createdAt`, `updatedAt` 처리 방식을 결정한다.
- 필수 필드, 날짜 형식, 프로젝트 참조 무결성, 알 수 없는 필드 처리 기준을 확정한다.

수동 테스트 후보:

- 내보낸 JSON 구조에 `format`, `schemaVersion`, `exportedAt`, `tasks`, `projects`, `appSettings`가 포함된다.
- 내보낸 JSON의 데이터 수가 화면과 일치한다.
- 내보내기 후 기존 IndexedDB 데이터가 변경되지 않는다.
- 잘못된 파일을 가져오거나 가져오기를 취소하면 기존 데이터가 유지된다.
- 서버 API, `localStorage`, 로그인, 클라우드 동기화가 추가되지 않는다.

구현 전 중단 조건:

- DB schema 변경 또는 migration이 필요해지는 경우
- 파일 파싱 오류 처리 정책이 미정인 경우
- 가져오기 실패 시 기존 데이터 보존 방식이 미정인 경우
- 기존 데이터 손상 위험을 낮출 방어책이 없는 경우
- 서버 API, `localStorage`, 로그인, 클라우드 동기화, 알림 권한 요청, Capacitor가 필요해지는 경우

## 45. 오류/로딩/빈 상태 보강 정책

MVP 이후 JSON 내보내기 기능을 구현하기 전에 오류, 로딩, 빈 상태 UX를 먼저 정책으로 정리한다. 상세 기준은 [docs/error-loading-state-policy.md](docs/error-loading-state-policy.md)를 따른다.

현재 결정:

- 이번 단계에서는 기능을 구현하지 않고 정책만 문서화한다.
- 앱 초기 데이터 로딩, IndexedDB 초기화, rule-based 추천 업무 생성, 향후 JSON 내보내기/가져오기를 로딩 상태 후보로 둔다.
- IndexedDB 읽기/쓰기 실패, 업무/프로젝트 CRUD 실패, 추천 업무 생성 실패, 향후 백업 파일 생성 실패, 향후 가져오기 파일 파싱 실패를 오류 상태 후보로 둔다.
- 업무 없음, 프로젝트 없음 또는 기본 프로젝트만 있음, 검색 결과 없음, 필터 결과 없음, 추천 업무 없음, 지난 마감 업무 없음, 7일 이내 마감 업무 없음을 빈 상태 후보로 둔다.
...
6. JSON 가져오기/복원은 별도 단계에서 정책 확정 후 구현
...
- 앱 초기 로딩 중 화면이 깨지지 않는지 확인한다.
- 업무 없음, 검색 결과 없음, 필터 결과 없음 상태를 구분해 확인한다.
...
## 47. JSON export 전용 피드백 정책

JSON 내보내기 구현 전 사용자에게 보여줄 성공/실패 상태 메시지를 먼저 정리한다. 상세 기준은 [docs/data-backup-export-policy.md](docs/data-backup-export-policy.md)의 `JSON export 성공/실패 메시지 정책`을 따른다.

성공 메시지 정책:

- 파일 생성과 다운로드 시작이 정상적으로 수행되면 성공으로 본다.
- 성공 메시지는 SettingsView 내부의 export 버튼 근처에 짧게 표시한다.
- 내보낸 데이터 범위는 `업무, 프로젝트, 설정` 정도로 요약할 수 있다.
- 서버 전송이나 클라우드 백업으로 오해되지 않게 로컬 JSON 파일임을 분명히 한다.

실패 메시지 정책:

- IndexedDB 읽기 실패, JSON 생성 실패, 브라우저 다운로드 실패 가능성, 알 수 없는 오류를 실패 후보로 둔다.
- 실패해도 기존 IndexedDB 데이터는 변경하지 않는다.
- 실패 메시지는 재시도 안내를 포함하되 원인을 단정하지 않는다.

상태 표현 방식:

- 1차 후보는 SettingsView 내부의 버튼 근처 상태 메시지다.
- `alert`는 보조 후보로만 둔다.
- 전역 toast와 전역 loading overlay는 아직 보류한다.

다음 구현 후보:

1. SettingsView 안에 JSON 내보내기 버튼과 버튼 근처 상태 메시지 영역을 최소 구현한다.
2. export 성공 시 로컬 JSON 파일 다운로드를 시작하고 성공 메시지를 표시한다.
3. export 실패 시 기존 데이터를 변경하지 않고 실패 메시지를 표시한다.
4. JSON import는 별도 단계로 유지한다.
...
## 48. JSON export 수동 확인 결과

`3d209e2 Add JSON data export` 기준으로 JSON export 성공 경로를 브라우저에서 직접 확인했다.
...
- 실패 시 기존 IndexedDB 데이터가 유지되는지는 실패 경로 재현 방법을 마련한 뒤 확인한다.
- 성공 메시지가 서버 전송이나 클라우드 백업으로 오해되지 않는지의 문구 검토는 별도 UX 확인 항목으로 남긴다.
- JSON import/복원은 아직 구현하지 않았고, 별도 정책 확정 후 진행한다.

현재 판정:

- JSON export 1차 구현은 성공 경로 기준으로 수동 확인 완료 상태다.
- 실패 경로는 확인 필요로 남아 있으므로, 이후 오류 재현 절차 또는 개발용 테스트 방법을 먼저 정리한다.

다음 작업 후보:

1. JSON export 실패 경로 재현/확인 절차 문서화
2. JSON import/복원 정책 구체화
3. 업무 필터/정렬 UX 개선 후보 정리
4. IndexedDB schema migration 규칙 문서화

## 49. JSON export 실패 경로 재현/기록 정책

JSON export 성공 경로는 수동 테스트로 통과했지만, 실패 경로는 일반 브라우저 환경에서 안정적으로 재현하지 못했다.
...
- 일반 브라우저 화면 조작만으로는 대부분의 실패 후보를 안정적으로 재현하기 어렵다.
- 개발자 도구에서 브라우저 API를 강제로 바꾸거나 코드에 실패 주입을 하지 않는 한 실패 메시지 표시를 직접 확인하기 어렵다.
- 현재 작업에서는 실패를 만들기 위한 src 코드 변경을 하지 않는다.

현재 단계의 처리 기준:

- 실패 경로는 `수동 재현 어려움 / 코드 경로상 try-catch 확인 필요`로 기록한다.
- 성공 경로 통과를 기준으로 JSON export 1차 구현은 완료로 본다.
- 실패 메시지는 현재 `SettingsView`의 `catch` 경로에서 표시되도록 되어 있으나, 실제 실패 재현 확인은 보류한다.
- 실패 상황에서도 기존 IndexedDB 데이터를 변경하지 않는 정책을 유지한다.
- JSON import/복원은 아직 구현하지 않고 별도 정책 확정 후 진행한다.

향후 보강 후보:

1. `exportData` 유틸 단위 테스트 도입 정책을 문서화한다.
2. Blob/object URL/download link 처리를 download helper로 분리할지 검토한다.
3. 실패 주입 가능한 테스트 함수 또는 테스트 hook 도입 여부를 결정한다.
4. Vitest 같은 테스트 도구 도입 여부를 별도 작업으로 판단한다.
5. 필요하면 Chrome/Edge 등 브라우저별 다운로드 동작을 수동 확인한다.

다음 단계 후보:

1. JSON export 1차 완료 상태를 유지하고 import/복원 정책을 구체화한다.
2. export 실패 경로 테스트 도구 도입 정책을 문서화한다.
3. 업무 필터/정렬 UX 개선 후보를 정리한다.

## 50. JSON import/복원 정책

JSON import/복원은 아직 구현하지 않는다. 데이터 손상 위험이 있으므로 상세 기준은 [docs/data-import-restore-policy.md](docs/data-import-restore-policy.md)를 기준으로 한다.
...
- 전체 덮어쓰기: 1차 실제 복원 후보. 단순하지만 현재 데이터 삭제 위험이 있으므로 백업/확인/rollback 정책이 필요하다.
- 병합: 중복 ID, 충돌 우선순위, project 참조 무결성 위험이 커서 1차 범위에서 제외한다.
- 선택적 복원: task/project/settings 관계가 깨질 수 있어 후순위로 둔다.
...
- `projects` 배열 여부
- `appSettings` 객체 여부
- task/project/appSettings 필수 필드 존재 여부
- task `status`와 `priority` 허용 값 여부
- task `projectId`가 백업 파일 안의 project를 참조하는지 여부
- 날짜 문자열 형식 검토
...
- 실제 복원 전 현재 데이터 백업 안내 또는 자동 백업 정책을 확정한다.
- 부분 실패 가능성이 있으면 transaction 또는 rollback 기준을 먼저 확정한다.

보류/중단 조건:

- DB schema 변경 또는 migration이 필요하다.
- rollback 정책이 정해지지 않았다.
- 중복 ID 정책이 정해지지 않았다.
- `appSettings` 덮어쓰기 정책이 정해지지 않았다.
- `App.css` 대규모 수정이 필요하다.
- 서버 API, `localStorage`, 로그인, 클라우드 동기화, 알림 권한 요청, Capacitor가 필요하다.
